...
今回はこの祝日データを JavaScript に読み込んで使っていきましょう。
...
必要なのは祝日の年月日だけです。祝日の名前は使用しませんので、年月日だけを使えるようにしましょう。
...
~~~
const str = "apple,orange,strawberry";
let array = str.split(',');
console.log(array);
~~~
...
function isHoliday(holidays, year, month, day) {        //
  ...
  return false;                                         // 一致しなかったら false を返す
}
~~~
...
function calendar(year) {
  ...
  for (let d = 1; d <= 31; d++) {
    ...
    if (isHoliday(holidays, year, 1, d) === true) {       // ここから追加
